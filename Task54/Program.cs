...
//3. Метод, упорядочивающий по убыванию элементы каждой строки двумерного массива из п.1
void RowsElementsDescendingOrder(int[,] matrix)
{
    for (int i = 0; i < matrix.GetLength(0); i++)
    {
        for (int j = 0; j < matrix.GetLength(1); j++)
...
RowsElementsDescendingOrder(array2D);
PrintMatrix(array2D);